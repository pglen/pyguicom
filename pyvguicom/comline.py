...
def print_exception(xstr, fp=sys.stdout):
...
def  test_exc():
    try:
        ...
        fp = StringIO()
        print_exception("Hello", fp)
        fp.seek(0)
        txt = fp.read()
        print(txt)
        fp.close()
        ret = txt.find("Hello <class 'ValueError'> Test")
        assert 0 == ret
...
def  test_long():
    ...
    args = conf.comline(( "-d 1", "hello",))
    #print("args:", args)
    #print("vars:", end = " ")
    #conf.printvars()
    assert conf.pgdebug == 1
